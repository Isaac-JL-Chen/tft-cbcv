...
Lambda = tf.keras.layers.Lambda # any function


# Default input types as integer.
...
def gated_residual_network(x,
  ...
  # Apply feedforward network: hidden to gating
  hidden = linear_layer(
      hidden_layer_size,
      activation=None,
...
class InterpretableMultiHeadAttention():
  ...
  def __call__(self, q, k, v, mask=None):
    ...
    for i in range(n_head):
      ...
      head, attn = self.attention(qs, ks, vs, mask) # scaled dot product
    ...
    outputs = K.mean(head, axis=0) if n_head > 1 else head # average of multi-head
    outputs = self.w_o(outputs) # dense
...
class TemporalFusionTransformer(object):
  ...
  Attributes:
    ...
    -use_cudnn: Whether to use Keras CuDNNLSTM (faster, only be run on GPU) or standard LSTM layers
  ...
  def __init__(self, raw_params, use_cudnn=False):
    ...
    self.name = self.__class__.__name__ # TemporalFusionTransformer

    params = dict(raw_params)  # copy locally

    #========(from data_formatters.base)============#
    # Data parameters 
    self.time_steps = int(params['total_time_steps']) # total num of time steps
    self.input_size = int(params['input_size']) # length of input columns
    self.output_size = int(params['output_size']) # length of target
    self.category_counts = json.loads(str(params['category_counts'])) # number of categories per relevant input for keras embedding layers
    self.n_multiprocessing_workers = int(params['multiprocessing_workers']) # num of cpus for data processing
    ...
    self.hidden_layer_size = int(params['hidden_layer_size']) # Internal state size of TFT
    ...
    self.num_encoder_steps = int(params['num_encoder_steps']) # length of LSTM encoder (i.e. history)
    self.num_stacks = int(params['stack_size']) # Number of self-attention layer
    self.num_heads = int(params['num_heads']) # Number of heads for interpretable mulit-head attention
    ...
    ## printing parameters ============================================
    print('\n\n*** {} params in initiating process***'.format(self.name))
    for k in params:
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    Returns:
      ...
      - unknown_inputs : time varying future unknown features
      - known_combined_layer : time varying future known features
      - obs_inputs : target
    ...
    for i in range(num_categorical_variables):
      embedding = tf.keras.Sequential([
          ...
          tf.keras.layers.Embedding(
              input_dim=self.category_counts[i],
              output_dim=embedding_sizes[i],
              input_length=time_steps,
              dtype=tf.float32) # output: (None, time_steps, embedding_sizes[i])
      ])
    ...
    def convert_real_to_embedding(x):
      ...
      return tf.keras.layers.TimeDistributed( # apply a layer to each time step.
          tf.keras.layers.Dense(self.hidden_layer_size))(
              x)


    # Targets (continuous variable)
  ...
  def _batch_data(self, data):
    ...
    # Combine all data
    data_map = {k:[data_map[k][i] for i in range(len(data_map[k])) if data_map[k][i] is not None] for k in data_map if data_map[k] is not None}
    for k in data_map:
  ...
  def _build_base_graph(self):
    ...
    # Embeddings. linear transformation (continuous) and entity embedding (categorical)
    ...
    # Isolate only known future inputs. (encoder length ~ end)
    ...
    def static_combine_and_mask(embedding):
      ...
      # GRN based weight * GRN based embedding vector
    ...
    # static context embdding is used in four parts
    ...
    def lstm_combine_and_mask(embedding):
      ...
      Returns:
        ...
        -temporal_ctx : temporal GRN based vector
        -sparse_weights : temporal GRN based weights
        -static_gate : wieghts that generate sparse_weights
      """
    ...
    # temporal GRN based outcome vector and weight (for interpretability)
    ...
    def get_lstm(return_state):
      ...
      if self.use_cudnn:
        lstm = tf.keras.layers.CuDNNLSTM(
            self.hidden_layer_size,
            return_sequences=True, ## all hidden state over all time step
    ...
    # historical: all hidden state, last hidden state, last cell state
    ...
    # future: all hidden state
    future_lstm = get_lstm(return_state=False)(future_features, 
    ...
    with tf.variable_scope(self.name):
      ...
      class QuantileLossCalculator(object):
        ...
        def quantile_loss(self, a, b):
          ...
          # self.file_writer.add_summary(loss)
          return loss
...
### acquisition & initial spend & censored cohort
### 3*2*3*3 = 54 cases
  @classmethod
  def get_hyperparm_choices(cls):
    """Returns hyperparameter ranges for random search."""
