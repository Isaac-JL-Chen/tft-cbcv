...
def main(expt_name, 
  ...
  Args:
    ...
    gpu_number: which device in multiple GPUs
  ...
  os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_number) # set gpu 0 as a default
  ...
  if use_gpu:
    ...
    tf_config = utils.get_default_tensorflow_config(tf_device="gpu", gpu_id=gpu_number)
...
if __name__ == "__main__":
  ...
  def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="GPU device number. Default={}".format("0"))
    parser.add_argument(
        "--output_folder",
        metavar="f",
        type=str,
    ...
    return args.expt_name, args.gpu_num, root_folder, args.use_gpu == "yes", \
        args.restart_hyperparam_opt

  # Load settings for default experiments
  name, gpu_num, folder, use_tensorflow_with_gpu, restart = get_args()
  ...
  main(
      ...
      restart_opt=restart,
      model_folder=os.path.join(config.model_folder, name + "_hyp_optim"),
